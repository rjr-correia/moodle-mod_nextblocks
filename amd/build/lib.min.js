 ...
 */define ("mod_nextblocks/lib", ['mod_nextblocks/codestring', 'mod_nextblocks/codestringPython', 'core/str'], function (CodeString, CodeStringPython, str) {
  function generateFunctionCode(block) {
    Blockly.JavaScript.blockToCode(block);
    let functionName = '%' + Blockly.JavaScript.nameDB_.getName(block.getFieldValue('NAME'), Blockly.Procedures.NAME_TYPE);
  ...
  function generateFunctionCodePython(block) {
    ...
    let functionName = '%' + Blockly.Python.nameDB_.getName(block.getFieldValue('NAME'), Blockly.Procedures.NAME_TYPE);
    functionName = functionName.substring(0, functionName.length - 1);
  ...
  return {
    ...
    testsAccordion: async function (results, testsJSON) {
      ...
      for (let i = 0; i < testCaseCount; i++) {
        ...
        if (results === null || results[i] === undefined) {
          ...
          accordion += '<span class="badge badge-warning float-right" style="color: black !important;">' + strtesterror + '</span>';
        } else if (results[i] === testsJSON[i].output) {
          accordion += '<span class="badge badge-success float-right" style="color: green !important;">' + strtestpassed + '</span>';
        } else {
          accordion += '<span class="badge badge-danger float-right" style="color: red !important;">' + strtestfailed + '</span>';
        }
        accordion += '</summary>';
    ...
    errorPrevention: code => {
      let strerrormaxtime = "";
      str.get_string('errormaxtime', 'mod_nextblocks').then(function (text) {
        strerrormaxtime = text;
      });
      code = code.replace(/((?:while|for)\s*\([^)]*\)\s*\{)/g, "$1\nif(loopIterations++>MAX_ITERATIONS) return outputString = " + "\"" + strerrormaxtime + "\";");
      return code;
    },
